...
import { getProvincetypeInfoLevel1, getProvinceTypeSelect1 } from "@/Common/enum/country/AdministrativeUnitEnum";
...
const loadDatas = async () => {
  try {
    ...
    const res = await AdministrativeUnitAPI.GetAll({
      ...filters.value,
...
watch(
  ...
  () => {
    filters.value.id == props.country?.id;
    loadDatas();
  }
);
